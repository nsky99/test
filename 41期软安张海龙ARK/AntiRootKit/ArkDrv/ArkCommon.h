...
{
  ULONG ulLen;
  UCHAR szInitialized[4];
  PVOID pSsHandle;
  LIST_ENTRY InLoadOrderModuleList;
  LIST_ENTRY InMemoryOrderModuleList;
  LIST_ENTRY InInitializationOrderModuleList;
  PVOID pEntryInProgress;
  UCHAR szShutdownInProgress[4];
  PVOID pShutdownThreadId;
...
typedef struct _IDTR {
  UINT16 uLimit;      // IDT范围
  UINT16 uLowBase;    // IDT低基址
  UINT16 uHighBase;   // IDT高基址
}IDTR, * PIDTR;
...
typedef struct _GDTR {
  UINT16 uLimit;      // GDT范围
  UINT16 uLowBase;    // GDT低基址
  UINT16 uHighBase;   // GDT高基址
}GDTR, * PGDTR;
